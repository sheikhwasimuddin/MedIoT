...
export default function AuthCard() {
  ...
  return (
    <Card className="w-full max-w-md mx-auto border-white/10 bg-slate-950/70 text-white shadow-[0_0_80px_rgba(34,211,238,0.08)] backdrop-blur-2xl">
      <CardHeader className="text-center space-y-3">
        <div className="flex justify-center">
          <div className="grid h-20 w-20 place-items-center rounded-3xl border border-white/10 bg-white/10">
            <img src="/logo.png" alt="MedIoT Logo" className="h-14 w-14 rounded-2xl object-cover" />
          </div>
        </div>

        <CardDescription className="text-slate-300">Securely access your health and device data</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="login" className="w-full" onValueChange={(value) => { setActiveTab(value); handleStateReset(); }}>
          <TabsList className="grid grid-cols-2 w-full border border-white/10 bg-white/5">
...
>
  Forgot Password?
            ...
            {error && activeTab === 'login' && <p className="text-sm text-center text-red-300">{error}</p>}
            <Button className="w-full font-semibold bg-cyan-300 text-slate-950 hover:bg-white" onClick={handleLogin} disabled={isLoading}>
          ...
          <TabsContent value="register" className="space-y-4 pt-4">
            ...
            {error && activeTab === 'register' && <p className="text-sm text-center text-red-300">{error}</p>}
            <Button className="w-full font-semibold bg-cyan-300 text-slate-950 hover:bg-white" onClick={handleRegister} disabled={isLoading}>
        ...
        <div className="space-y-2">
          <Button variant="outline" className="w-full border-white/10 bg-white/5 font-semibold text-white hover:bg-white hover:text-slate-950" onClick={handleGoogleSignIn} disabled={isLoading}>
                {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <GoogleIcon className="mr-2 h-4 w-4" />}
                Google
            </Button>
           <Button variant="outline" className="w-full border-white/10 bg-white/5 font-semibold text-white hover:bg-white hover:text-slate-950" onClick={handleGithubSignIn} disabled={isLoading}>
